...
func runCreateSSOUsers(cmd *cobra.Command, emailFlags []string, inputFile, groupName string) error {
	...
	ctx := cmd.Context()
	ssoClient := newSSOAdminClient(cfg)
	identityStoreClient := newIdentityStoreClient(cfg)
	...
	if runtime.Options.DryRun {
		return cliutil.WriteDataset(cmd, runtime, []string{"email", "display_name", "group", "action"}, rows)
	}
	...
	for i, email := range emails {
		...
		userOut, createErr := identityStoreClient.CreateUser(ctx, &identitystore.CreateUserInput{
			IdentityStoreId: cliutil.Ptr(identityStoreID),
			UserName:        cliutil.Ptr(email),
			DisplayName:     cliutil.Ptr(displayName),
			Name: &identitystoretypes.Name{
				GivenName:  cliutil.Ptr(firstName),
				FamilyName: cliutil.Ptr(lastName),
			},
			Emails: []identitystoretypes.Email{{
				Value:   cliutil.Ptr(email),
				Type:    cliutil.Ptr("Work"),
				Primary: true,
			}},
		})
		if createErr != nil {
			rows[i][3] = cliutil.FailedActionMessage(awstbxaws.FormatUserError(createErr))
			continue
		}
		...
		_, membershipErr := identityStoreClient.CreateGroupMembership(ctx, &identitystore.CreateGroupMembershipInput{
			...
			MemberId:        &identitystoretypes.MemberIdMemberUserId{Value: cliutil.PointerToString(userOut.UserId)},
		})
...
func resolveIdentityStoreID(ctx context.Context, client SSOAdminAPI) (string, error) {
	...
	for _, instance := range instances {
		id := cliutil.PointerToString(instance.IdentityStoreId)
		if id != "" {
			return id, nil
		}
	}
...
func resolveIdentityStoreGroupID(ctx context.Context, client IdentityStoreAPI, identityStoreID, groupName string) (string, error) {
	groups, err := awstbxaws.CollectAllPages(ctx, func(callCtx context.Context, nextToken *string) (awstbxaws.PageResult[identitystoretypes.Group], error) {
		page, listErr := client.ListGroups(callCtx, &identitystore.ListGroupsInput{
			...
			Filters: []identitystoretypes.Filter{{
				AttributePath:  cliutil.Ptr("DisplayName"),
				AttributeValue: cliutil.Ptr(groupName),
			}},
			NextToken: nextToken,
		})
		if listErr != nil {
	...
	for _, group := range groups {
		if strings.EqualFold(cliutil.PointerToString(group.DisplayName), groupName) {
			return cliutil.PointerToString(group.GroupId), nil
		}
	}
